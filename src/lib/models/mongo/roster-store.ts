...
export const rosterStore = {
  async find(): Promise<Roster[]> {
    const rosters = await RosterMongoose.find().populate("donor").populate("candidate").lean();
    return rosters;
  },

  async findBy(agencyId: string): Promise<Roster[]> {
    const roster = await RosterMongoose.find({ agency: agencyId });
    return roster;
  },
  ...
  async add(roster: Roster): Promise<Roster | null> {
    ...
    const populatedRoster = await RosterMongoose.findById(newRoster._id).populate("donor").populate("candidate").lean();
